    ...
    const fetchData = async () => {
      try {
        ...
        const [vehicleRes, metricsRes, tripsRes] = await Promise.all([
          ...
          axios.get(`/api/vehicles/${vehicleId}/trips?limit=30`)
        ]);
            ...
            {metrics.length > 0 ? (
              <ResponsiveContainer width="100%" height={300}>
                <LineChart data={metrics}>
                  ...
                  <Tooltip contentStyle={{ background: 'rgba(10, 14, 39, 0.9)', border: '1px solid rgba(255,255,255,0.2)' }} />
                  <Legend />
                  <Line type="monotone" dataKey="efficiency_wh_per_km" stroke="#4caf50" name="Wh/km" dot={false} />
                </LineChart>
              </ResponsiveContainer>
            ) : (
              <p className="text-muted">No data available</p>
          ...
          <div className="trips-table">
            ...
            {trips.map(trip => (
              <div key={trip.id} className="trips-row">
                ...
                <div className="col-distance">{trip.distance_km.toFixed(1)} km</div>
                <div className="col-efficiency">{trip.efficiency_wh_per_km.toFixed(0)} Wh/km</div>
